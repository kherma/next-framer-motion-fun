...
const VorpStatus = ({ completedActions }) => {
  return (
    <div className="flex gap-2 justify-between items-center py-2 w-full sm:gap-8 sm:py-4 xl:py-2">
      <div className="p-1 w-full h-12 bg-bgViolet-500 rounded-full border-4 border-bgViolet-400 sm:h-20 xl:h-12 2xl:h-16">
          ...
          <AiFillHeart
            ...
            className={`w-10 h-10 ${
              completedActions[item] !== undefined
                ? "text-red-500"
                : "text-bgViolet-400"
            } sm:w-16 sm:h-16 xl:w-10 xl:h-10 2xl:w-16 2xl:h-16`}
          />
        ))}
      </div>
    </div>
